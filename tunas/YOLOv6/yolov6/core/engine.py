...
from yolov6.utils.torch_utils import EarlyStopping

class Trainer:
    def __init__(self, args, cfg, device, teacher_cfg=None):
        self.args = args
        self.cfg = cfg
        self.device = device
        ...
        # whether write eval images into tb or not
        self.write_valimg_tb = args.write_valimg_tb
    ...
    def train(self):
        try:
            ...
            for self.epoch in range(self.start_epoch, self.max_epoch):
                ...
                if self.args.evolve and self.stop:
                    break
                    
            if self.args.evolve:
                return self.evolve_evaluate_results
            else:
    ...
    def train_in_steps(self, epoch_num):
        ...
        if self.write_trainbatch_tb and self.main_process and self.step == 0 and not self.args.evolve:
    ...
    def eval_and_not_save(self):
        if self.main_process:
            self.ema.update_attr(self.model, include=['nc', 'names', 'stride']) # update attributes for ema model
            self.eval_model()
            map50, map50_95, precision, recall, _ = self.evaluate_results
            
            self.fi = fitness(np.array([precision, recall, map50, map50_95]).reshape(1, -1))  # weighted combination of [P, R, mAP@.5, mAP@.5-.95]
            if self.fi > self.best_fitness:
                self.best_fitness = self.fi
                self.evolve_evaluate_results = (precision, recall, map50, map50_95)
                
            self.stop = self.stopper(epoch=self.epoch, fitness=self.fi)  # early stop check
    ...
    def eval_and_save(self):
        ...
        if self.main_process:
            ...
            if self.ap >= self.best_ap:
                ...
                files = [osp.join(self.save_dir, f) for f in files if osp.exists(osp.join(self.save_dir, f))]  # filter
                for f in files:
                    self.tblogger.add_image(os.path.basename(f).split('.')[1], cv2.imread(f)[..., ::-1], self.epoch, dataformats='HWC')
    ...
    def eval_model(self):
        ...
        self.evaluate_results = results #results[:2]
    ...
    def train_before_loop(self):
        ...
        self.evaluate_results = (0, 0, 0, 0, 0) # AP50, AP50_95, P, R, F1
        self.evolve_evaluate_results = (0, 0, 0, 0)
        ...
        if self.args.distill:
            try:
                ...
                distill_weight={'class': 2.0,'dfl': 1.0,}
